...
# ==========================================
#         ⚙️ CONFIGURATION N'SYMBIOS V8.1
# ==========================================
...
def logger_requete(question, resultats_vecteurs, reponse_llm, temps_recherche):
    """Enregistre chaque interaction pour le profiling futur (Phase 2)"""
    log_entry = {
...
# ==========================================
#    🧠 LE MOTEUR HYBRIDE (VECTEUR + BM25)
# ==========================================

def recherche_hybride(question, table, bm25, corpus_textes, corpus_chemins, top_n=10):
    ...
    # 3. Fusion Mathématique RRF (Reciprocal Rank Fusion)
    rrf_scores = {}
    docs_metadata = {}
    k = 60 # Constante standard de lissage RRF
    
    # Ajout des scores sémantiques
    for rank, res in enumerate(res_vector):
        doc_id = hash(res['texte'])
        rrf_scores[doc_id] = rrf_scores.get(doc_id, 0.0) + (1.0 / (k + rank + 1))
        docs_metadata[doc_id] = res
        
    # Ajout des scores lexicaux purs
    for rank, idx in enumerate(top_bm25_indices):
        texte = corpus_textes[idx]
        doc_id = hash(texte)
        rrf_scores[doc_id] = rrf_scores.get(doc_id, 0.0) + (1.0 / (k + rank + 1))
        
        # Si le document n'a pas été trouvé par le vecteur, on l'ajoute au dictionnaire
    ...
    # Tri final pour isoler les 10 meilleurs absolus
...
if TABLE_NAME in db.table_names():
    ...
    print("   ℹ️ Base V8 vierge. Création de l'index à Chunking Adaptatif.")
...
if total_nouveaux_fichiers > 0 or len(fichiers_indexes) == 0:
    ...
    print(f"\n\n✅ Base de données V8 mise à jour ! (Erreurs de vectorisation : {erreurs_vectorisation})")
...
if HAS_BM25:
    ...
    # On charge la base en mémoire vive pour la recherche par mots clés
    df_all = table.search().limit(100000).to_pandas()
    corpus_textes = df_all['texte'].tolist()
    corpus_chemins = df_all['chemin_absolu'].tolist()
    
    # Tokenisation robuste (enlève la ponctuation)
    tokenized_corpus = [re.findall(r'\w+', doc.lower()) for doc in corpus_textes]
    bm25 = BM25Okapi(tokenized_corpus)
else:
    print("\n⚠️ Module 'rank_bm25' manquant. Moteur hybride désactivé, recherche sémantique seule.")
    bm25, corpus_textes, corpus_chemins = None, None, None
...
if HAS_BM25:
    resultats = recherche_hybride(question, table, bm25, corpus_textes, corpus_chemins, top_n=10)
else:
...
for res in resultats:
    # Affiche le score RRF si l'hybridation est active, sinon la distance LanceDB classique
