        ...
        "top_documents": [
            {
                ...
                "vector_distance": res.get('_distance', 0.0), 
                "bm25_score": None, 
...
# ==========================================
#    🔪 MOTEUR DE CHUNKING ADAPTATIF (V8)
# ==========================================
...
class MarkdownAdaptiveChunker:
    ...
    def _process_section(self, titre, contenu, projet, nom_fichier):
        ...
        # ==========================================
        # 🛡️ LA GUILLOTINE ABSOLUE (Anti-Crash 500)
        # ==========================================
        MAX_CHARS = 800 # Seuil brutalement abaissé pour garantir < 512 tokens
        chunks_securises = []
        
        for chunk in chunks_subdivises:
            if len(chunk) > MAX_CHARS:
                # 🔴 CORRECTION : On réduit la fenêtre de découpe pour laisser la place à l'étiquette contextuelle
                taille_utile = MAX_CHARS - len(prefixe_contexte) - 15 
                if taille_utile < 100: taille_utile = 100 # Garde-fou
...
def get_vector_optimized(text, is_query=False):
    ...
    try:
        # 🔴 CORRECTION FINALE : Verrou infranchissable à 800 caractères.
...
if vecteur_question:
    ...
    print("❌ Impossible de vectoriser la question.")
    resultats = []
...
for res in resultats:
    contexte_global += f"- (Score: {res['_distance']:.3f}) {res['texte']}\n"
